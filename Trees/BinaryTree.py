...
sibling, children, num_children
is_root, is_leaf					O(1)
depth(p)							O(dP + 1)
height								O(n)
add_root, add_left, add_right 		O(1)
replace, delete, attach

"""

import AbstractBinaryTreeBase

class Tree(AbstractBinaryTreeBase.Tree):
	...
	class Position(BinaryTreeBase.Tree.Position):
	...
	def right(self, p):
		"""Return the Position of p's parent (or None if no right child)."""
		node = self._validate(p)
	...
	def _add_right(self, p, e):
		...
		node = self._validate(p)
		if node._right is not None: raise ValueError('Left right exists')
		self._size += 1
